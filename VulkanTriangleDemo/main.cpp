...
private:
    ...
    void createSwapChain() {
        ...
        createInfo.oldSwapchain = VK_NULL_HANDLE; // This is the first swap chain, so we don't have an old one
    ...
    void createRenderPass() {
		...
		VkSubpassDependency dependency{}; // Dependency for the subpass
    ...
    void createGraphicsPipeline() {
		...
		// Viewport state uses dynamic state
		...
		// Color blending state performs no blending as configure attachment blend state without blending
    ...
    void drawFrame() {
		vkWaitForFences(device, 1, &inFlightFence, VK_TRUE, UINT64_MAX); // wait until the previous frame has finished, so that the command buffer and semaphores are available to use
		vkResetFences(device, 1, &inFlightFence); // reset the fence to the unsignaled state
    ...
    void createLogicalDevice() {
        ...
        if (indices.graphicsFamily.value() == indices.presentFamily.value()) {
            // If graphics and present families are the same, we can create a single queue
            // No need to create separate queues for graphics and presentation
            VkDeviceQueueCreateInfo queueCreateInfo{};
            queueCreateInfo.sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO;
			queueCreateInfo.queueFamilyIndex = indices.graphicsFamily.value(); // Use the graphics family index for both graphics and presentation
            queueCreateInfo.queueCount = 1;

            float queuePriority = 1.0f;     // priority of the queue, higher values indicate higher priority
